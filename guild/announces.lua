...
function TI.AnnounceGuildMemberVeteranLeveled(eventCode, guildId, AccName, charName, newLvl)
	if( not TI:getShowStatus(guildId, TI.SETTINGS_LEVEL)) then return end
	if( TI:isPlayerSelf(AccName) ) then return end
	if( TI:isPlayerOffline(AccName, guildId)) then return end
	...
	TI:showAnnouncement(TI.locale.templates.new_veteran, name, guildName, newLvl)
end
...
function TI.ANNOUNCES.Initialize()
	...
	EVENT_MANAGER:RegisterForEvent("TGI_GuildMemberVeteranLeveled", EVENT_GUILD_MEMBER_CHARACTER_CHAMPION_POINTS_CHANGED, TI.AnnounceGuildMemberVeteranLeveled)--
